...
async function main() {
  ...
  async function seedRoleBatch(roleCode: keyof typeof ROLES, count: number, prefix: string) {
  ...
  const projectCoordinators = await seedRoleBatch("PROJECT_COORDINATOR", 2, "pc");
