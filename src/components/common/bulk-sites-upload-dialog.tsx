...
export type BulkSitesUploadDialogProps = {
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  onUploadSuccess?: () => void;
};

export function BulkSitesUploadDialog({ open, onOpenChange, onUploadSuccess }: BulkSitesUploadDialogProps) {
  ...
  async function handleDownloadTemplate() {
    ...
    try {
      ...
      a.href = url;
      a.download = 'sites_template.xlsx';
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const response = await fetch('/api/sites/upload', { method: 'POST', body: formData });
      const data = await response.json().catch(() => null);
      if (!response.ok) {
        let errorMessage = data?.message || data?.error || 'Failed to upload sites data';
      ...
      toast.success(data?.message || `Successfully uploaded ${data?.count || 0} site records`);
      ...
      const errorMessage = (error as Error).message || 'Failed to upload sites data';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Upload Sites</DialogTitle>
          <DialogDescription>
            Download the Excel template, fill in site details (Site*, Company*, optional address fields), and upload it back.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">Required columns: Site*, Company*. Optional: Status, Site Code, Short Name, Address fields, PAN/GST/TAN/CIN, and one Contact Person.</p>
          </div>
