...
export function BulkVendorsUploadDialog({ open, onOpenChange, onUploadSuccess }: BulkVendorsUploadDialogProps) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const response = await fetch('/api/vendors/upload', { method: 'POST', body: formData });
      const data = await response.json().catch(() => null);
      if (!response.ok) {
        let errorMessage = data?.message || data?.error || 'Failed to upload vendors';
      ...
      toast.success(data?.message || `Successfully uploaded ${data?.count || 0} vendors`);
      ...
      const errorMessage = (error as Error).message || 'Failed to upload vendors';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Download the Excel template, fill in vendor details, and upload it back.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">Columns: vendorName*, contactPersonName*, addressLine1*, addressLine2, state*, city*, pincode, mobile1, mobile2, email, alternateEmail1..4, landline1..2, bankName, branchName, branchCode, accountNumber, ifscCode, panNumber, gstNumber, vatTinNumber, cstInNumber, cinNumber, serviceTaxNumber, stateCode.</p>
          </div>
