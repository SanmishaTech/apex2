...
export type BulkEmployeesUploadDialogProps = {
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  onUploadSuccess?: () => void;
};

export function BulkEmployeesUploadDialog({ open, onOpenChange, onUploadSuccess }: BulkEmployeesUploadDialogProps) {
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const response = await fetch('/api/employees/upload', { method: 'POST', body: formData });
      const data = await response.json().catch(() => null);
      if (!response.ok) {
        let errorMessage = data?.message || data?.error || 'Failed to upload employees data';
      ...
      toast.success(data?.message || `Successfully uploaded ${data?.count || 0} employee records`);
      ...
      const errorMessage = (error as Error).message || 'Failed to upload employees data';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            Download the Excel template, fill the employee details (name, email, password, optional address/contact), and upload.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">Required: name, email, password (min 6). Optional: department, mobiles, address, state/city.</p>
          </div>
