...
export type BulkCitiesUploadDialogProps = {
  open?: boolean;
  onOpenChange?: (open: boolean) => void;
  onUploadSuccess?: () => void;
};

export function BulkCitiesUploadDialog({ open, onOpenChange, onUploadSuccess }: BulkCitiesUploadDialogProps) {
  ...
  async function handleDownloadTemplate() {
    setDownloading(true);
    try {
      const response = await fetch('/api/cities/template');
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const response = await fetch('/api/cities/upload', { method: 'POST', body: formData });
      const data = await response.json().catch(() => null);
      if (!response.ok) {
        let errorMessage = data?.message || data?.error || 'Failed to upload cities data';
      ...
      toast.success(data?.message || `Successfully uploaded ${data?.count || 0} city records`);
      ...
      const errorMessage = (error as Error).message || 'Failed to upload cities data';
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          <DialogTitle>Upload Cities</DialogTitle>
          <DialogDescription>
            Download the Excel template, fill in the cities (and optional state), and upload it back.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 py-4">
          <div className="flex flex-col gap-2">
            ...
            <p className="text-xs text-muted-foreground">Template columns: City* (required), State (optional, must exist).</p>
          </div>
