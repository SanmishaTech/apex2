...
const inputSchema = z.object({
  ...
  fax: z
    .string()
    .regex(/^[\d\s\-\+\(\)]{10,15}$/, "Invalid fax format")
    .optional()
    .or(z.literal("")),
...
export function BillingAddressForm({
  ...
  initial,
  onSuccess,
  redirectOnSuccess = "/billing-addresses",
  mutate,
  ...
  async function onSubmit(data: RawFormValues) {
    ...
    try {
      ...
      if (isCreate) {
        ...
        const res = await apiPatch("/api/billing-addresses", {
          id: initial.id,
          ...payload,
        });
    ...
    <>
      ...
      <Form {...form}>
        <AppCard>
          <AppCard.Header>
            <AppCard.Title>
              {isCreate ? "Create Billing Address" : "Edit Billing Address"}
            </AppCard.Title>
            <AppCard.Description>
              {isCreate
