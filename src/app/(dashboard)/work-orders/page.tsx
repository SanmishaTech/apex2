...
export default function WorkOrdersPage() {
  ...
  const { data, isLoading, mutate } = useSWR<WorkOrdersResponse>(
  ...
  const [poToDelete, setPoToDelete] = useState<WorkOrder | null>(null);
  ...
  const [poForRemark, setPoForRemark] = useState<WorkOrder | null>(null);
  const [remarkText, setRemarkText] = useState("");
  const [showBillStatusDialog, setShowBillStatusDialog] = useState(false);
  const [poForBillStatus, setPoForBillStatus] = useState<WorkOrder | null>(
    null
  );
  ...
  const openApproval = (
    ...
    if (key === "approve1") {
      pushWithScrollSave(`/work-orders/${id}/approve1`);
      return;
    }
    if (key === "approve2") {
      pushWithScrollSave(`/work-orders/${id}/approve2`);
      return;
    }
  ...
  const handleDeleteClick = (po: WorkOrder) => {
    setPoToDelete(po);
    setShowDeleteConfirm(true);
  };

  const handleRemarkClick = (po: WorkOrder) => {
    setPoForRemark(po);
    setRemarkText(po.remarks || "");
    setShowRemarkDialog(true);
  };

  const handleBillStatusClick = (po: WorkOrder) => {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await apiDelete(`/api/work-orders/${poToDelete.id}`);
  ...
  const handleStatusAction = async () => {
    ...
    try {
      await apiPatch(`/api/work-orders/${statusAction.po.id}`, {
  ...
  const handleRemarkSubmit = async () => {
    ...
    try {
      await apiPatch(`/api/work-orders/${poForRemark.id}`, {
        remarks: remarkText,
      });
  ...
  const handleBillStatusSubmit = async () => {
    ...
    try {
      await apiPatch(`/api/work-orders/${poForBillStatus.id}`, {
  ...
  const handlePrint = useCallback(
    async (po: WorkOrder) => {
      try {
        const res = await fetch(`/api/work-orders/${po.id}/print`);
        if (!res.ok) {
        ...
        link.href = url;
        link.download = `work-order-${po.workOrderNo}.pdf`;
  ...
  return (
    <div className="space-y-6">
        ...
        <AppCard.Content>
          <FilterBar title="Search & Filter">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <NonFormTextInput
                label="Search"
                placeholder="WO No., Quotation No., Note..."
                value={searchDraft}
      ...
      <ConfirmDialog
        ...
        description={`Are you sure you want to delete work order ${poToDelete?.workOrderNo}? This action cannot be undone.`}
        confirmText="Delete"
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              ...
              {statusAction.action === "suspend" && "Suspend Work Order"}
              {statusAction.action === "unsuspend" &&
                "Unsuspend Work Order"}
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <p>
                Are you sure you want to {statusAction.action} work order{" "}
                <strong>{statusAction.po?.workOrderNo}</strong>?
              </p>
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <p>
                Add remark for work order <strong>{poForRemark?.workOrderNo}</strong>
              </p>
              <Textarea
      ...
      >
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <p>
                Update bill status for work order <strong>{poForBillStatus?.workOrderNo}</strong>
              </p>
              <Textarea
