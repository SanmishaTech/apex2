...
export function NoticeForm({ mode, initial }: { mode: 'create' | 'edit'; initial?: NoticeInitialData | null; }) {
  useProtectPage();
  ...
  const [uploadedFileUrl, setUploadedFileUrl] = useState<string | null>(initial?.documentUrl || null);
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      noticeDate: initial?.noticeHead ? initial.noticeHead : formatDateForInput(new Date()),
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error('Invalid file type. Please select PDF, Word document, or image file.');
      return;
    }
                ...
                <div>
                  ...
                  <div className='mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-gray-400 transition-colors'>
                    <div className='space-y-1 text-center'>
                      ...
                      <p className='text-xs text-gray-500'>PDF, DOC, DOCX, PNG, JPG up to 20MB</p>
                    </div>
                  </div>
                  {uploadedFileUrl && (
                    <div className='mt-3 p-3 bg-green-50 border border-green-200 rounded-md'>
                      <div className='flex items-center'>
                        ...
                        <span className='ml-2 text-sm text-green-800'>File uploaded successfully</span>
                        <a
          ...
          <AppCard.Footer>
            <div className='flex gap-2'>
              ...
              <AppButton
                ...
                disabled={uploading}
              >
                {uploading ? 'Uploading...' : (mode === 'create' ? 'Save' : 'Update')}
              </AppButton>
            </div>
          </AppCard.Footer>
        </form>
