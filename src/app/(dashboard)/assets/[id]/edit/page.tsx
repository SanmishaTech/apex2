...
export default function EditAssetPage({ params }: EditAssetPageProps) {
  ...
  const {
    ...
    isLoading,
    mutate,
  } = useSWR<Asset>(id ? `/api/assets/${id}` : null, fetcher);
  ...
  const handleSubmit = async (data: any) => {
    try {
      ...
      if (data.assetCategoryId !== undefined)
      ...
      if (data.invoiceCopyUrl !== undefined)
        fd.append("invoiceCopyUrl", data.invoiceCopyUrl);
      ...
      // Revalidate SWR cache so reopening edit shows latest data
      if (mutate) {
        await mutate();
      }
      router.push("/assets");
