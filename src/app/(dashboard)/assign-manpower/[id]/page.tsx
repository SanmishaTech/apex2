...
export default function ViewAssignedManpowerPage({ params }: PageProps) {
  ...
  const { backWithScrollRestore } = useScrollRestoration('assign-manpower-view');
  ...
  const columns: Column<ViewRow>[] = [
    ...
    },
    {
      key: 'wage', header: 'Wage', sortable: false, className: 'text-right', cellClassName: 'text-right', accessor: (r) => (
        <input type='number' className='w-24 text-right border border-input bg-background text-foreground placeholder:text-muted-foreground rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring' value={String((edits[r.id]?.wage as any) ?? r.wage ?? '')}
    ...
    },
    {
      key: 'minWage', header: 'Min Wage', sortable: false, className: 'text-right', cellClassName: 'text-right', accessor: (r) => (
        <input type='number' className='w-24 text-right border border-input bg-background text-foreground placeholder:text-muted-foreground rounded px-2 py-1 focus:outline-none focus:ring-2 focus:ring-ring focus:border-ring' value={String((edits[r.id]?.minWage as any) ?? r.minWage ?? '')}
