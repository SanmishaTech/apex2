...
import type {
  ...
  AssignManpowerRequestItem,
} from "@/types/manpower-assignments";
...
export default function ViewAssignedManpowerPage({ params }: PageProps) {
  ...
  const { pushWithScrollSave, pushAndRestoreKey } = useScrollRestoration(
    "assign-manpower-view"
  );
  ...
  const { data: skillSets } = useSWR<{
    data: { id: number; skillsetName: string }[];
  ...
  function setField(id: number, key: keyof ViewEdit, value: any) {
    setEdits((prev) => ({
      ...prev,
      [id]: { ...(prev[id] || { manpowerId: id }), [key]: value },
    }));
  }
  ...
  const columns: Column<ViewRow>[] = [
    ...
    },
    {
      ...
      accessor: (r) => (
        <AppSelect
          value={(edits[r.id]?.skillSet as any) ?? r.skillSet ?? ""}
          onValueChange={(v) =>
            setField(r.id, "skillSet", v === "__none" ? "" : v)
          }
    ...
    },
    {
      ...
      accessor: (r) => (
        <input
          ...
          value={String((edits[r.id]?.wage as any) ?? r.wage ?? "")}
    ...
    },
    {
      ...
      accessor: (r) => (
        <input
          type="checkbox"
          checked={!!(edits[r.id]?.pf ?? r.pf)}
          onChange={(e) => setField(r.id, "pf", e.currentTarget.checked)}
        />
      ),
    },
    {
      ...
      accessor: (r) => (
        <input
          type="checkbox"
          checked={asBool(edits[r.id]?.esic ?? r.esic)}
          onChange={(e) => setField(r.id, "esic", e.currentTarget.checked)}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      accessor: (r) => (
        <input
          type="checkbox"
          checked={asBool(edits[r.id]?.pt ?? r.pt)}
          onChange={(e) => setField(r.id, "pt", e.currentTarget.checked)}
        />
      ),
    },
    {
      ...
      accessor: (r) => (
        <input
          type="checkbox"
          checked={asBool(edits[r.id]?.mlwf ?? r.mlwf)}
          onChange={(e) => setField(r.id, "mlwf", e.currentTarget.checked)}
        />
      ),
    },
  ];
  ...
  async function unassignSelected() {
    const manpowerIds = Object.entries(selected)
      .filter(([, v]) => v)
      .map(([k]) => Number(k));
    if (manpowerIds.length === 0) {
      toast.error("Select at least one manpower");
      return;
    }
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          (data as any)?.message || "Failed to unassign manpower"
        );
      }
  ...
  if (error)
    toast.error((error as Error).message || "Failed to load assigned manpower");
  ...
  return (
    <AppCard>
      <AppCard.Header>
        ...
        <AppCard.Description>
          Manage manpower currently assigned to this site.
        </AppCard.Description>
        <AppCard.Action>
          <AppButton size="sm" variant="secondary" onClick={goBack}>
            Back
          </AppButton>
        </AppCard.Action>
      </AppCard.Header>
      ...
      <AppCard.Footer className="justify-between flex-col sm:flex-row gap-4">
        ...
        <div className="flex items-center justify-between w-full sm:w-auto gap-4">
          <div className="text-sm text-muted-foreground">
            ...
            Changes: {Object.keys(edits).length}
          </div>
