...
interface RentDocumentUploadArrayProps {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  control: any;
}

export function RentDocumentUploadArray({ control }: RentDocumentUploadArrayProps) {
        ...
        <Fragment key={(field as Record<string, unknown>).fieldId as string ?? index}>
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-sm transition-all hover:shadow-lg sm:p-6">
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              ...
              <Controller
                control={control}
                name={`rentDocuments.${index}.documentName`}
                render={({ field: nameField, fieldState }) => (
                  <div className="flex-1 md:pl-6">
                    ...
                    <input
                      {...nameField}
                      type="text"
                      placeholder="e.g. Agreement, Receipts"
            ...
            <Controller
              control={control}
              name={`rentDocuments.${index}.documentUrl`}
