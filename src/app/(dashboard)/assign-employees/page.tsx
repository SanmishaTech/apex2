...
export default function AssignEmployeesSitesPage() {
  const { pushWithScrollSave } = useScrollRestoration("assign-employees-sites");
  ...
  type SiteRow = (SitesResponse["data"][number] & { _count?: { siteEmployees?: number } });
  const rows: SiteRow[] = (data?.data as SiteRow[]) ?? [];
  ...
  function toggleSort(field: string) {
    setQp(sort === field ? { order: order === "asc" ? "desc" : "asc" } : { sort: field, order: "asc" });
  }
  ...
  const columns: Column<SiteRow>[] = [
    ...
    { key: "assignedCount", header: "No. of Employees", sortable: false, accessor: (r: SiteRow) => r._count?.siteEmployees ?? 0, className: "whitespace-nowrap text-right" },
  ];
  ...
  return (
    <AppCard>
      <AppCard.Header>
        ...
        <AppCard.Description>Select a site to view or assign employees.</AppCard.Description>
      </AppCard.Header>
      <AppCard.Content>
