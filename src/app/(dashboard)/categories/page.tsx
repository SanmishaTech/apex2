...
export default function CategoriesPage() {
	...
	const query = useMemo(() => {
		...
		return `/api/categories?${sp.toString()}`;
	...
	if (error) {
		toast.error((error as Error).message || 'Failed to load categories');
	}
							...
							<div className='flex'>
								{can(PERMISSIONS.EDIT_CATEGORIES) && (
									<Link href={`/categories/${category.id}/edit`}>
										<EditButton tooltip='Edit Category' aria-label='Edit Category' />
									</Link>
								)}
								{can(PERMISSIONS.DELETE_CATEGORIES) && (
									<DeleteButton
										...
										description={`This will permanently remove category "${category.categoryName}". This action cannot be undone.`}
									/>
								)}
							</div>
						);
