...
type EditOverallSiteBudgetPageProps = {
  params: Promise<{ siteId: string; id: string }>;
};

export default function EditOverallSiteBudgetPage({
  params,
}: EditOverallSiteBudgetPageProps) {
  useProtectPage();
  ...
  useEffect(() => {
    async function fetchOverallSiteBudget() {
      try {
        const overallSiteBudget =
          await apiGet<OverallSiteBudgetsFormInitialData>(
