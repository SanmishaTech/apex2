...
function toSubmitPayload(data: RawFormValues) {
  return {
    ...
    fromTargetDate:
      data.fromTargetDate && data.fromTargetDate !== ""
        ? new Date(data.fromTargetDate).toISOString()
        : null,
    toTargetDate:
      data.toTargetDate && data.toTargetDate !== ""
        ? new Date(data.toTargetDate).toISOString()
        : null,
    dailyTargetQty:
      data.dailyTargetQty && data.dailyTargetQty !== ""
        ? data.dailyTargetQty
...
export function BoqTargetsForm({
  ...
  const form = useForm<RawFormValues>({
    ...
    defaultValues: {
      ...
      toTargetDate: initial?.toTargetDate
        ...
        : "",
      dailyTargetQty:
        initial?.dailyTargetQty != null ? String(initial.dailyTargetQty) : "",
    },
  });
  ...
  // Update selectedSiteId when form value changes
  useEffect(() => {
    if (watchedSiteId !== selectedSiteId) {
      // If user actually changed site (not initial mount), clear BOQ and Activity
      if (selectedSiteId) {
        setValue("boqId", "");
        setValue("activityId", "");
        setSelectedBoqId("");
  ...
  }, [watchedSiteId, selectedSiteId, setValue]);

  // Update selectedBoqId when form value changes
  useEffect(() => {
    // Only reset activity when BOQ actually changes after initial selection.
    // On initial mount, selectedBoqId is '', so we should not clear a prefilled activityId.
    if (watchedBoqId !== selectedBoqId) {
      // If there was a previously selected BOQ, clear the activity.
      if (selectedBoqId) {
        setValue("activityId", "");
      }
    ...
    <>
      ...
      <Form {...form}>
        <AppCard>
          <AppCard.Header>
            <AppCard.Title>
              {isCreate ? "Create BOQ Target" : "Edit BOQ Target"}
            </AppCard.Title>
            <AppCard.Description>
              {isCreate
                ? "Add a new BOQ Target."
                : "Update BOQ Target details."}
            </AppCard.Description>
          </AppCard.Header>
