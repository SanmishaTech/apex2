...
const inputSchema = z.object({
  ...
  mobile1: z.string().regex(/^\d{10}$/, 'Mobile number must be 10 digits').optional().or(z.literal('')),
  mobile2: z.string().regex(/^\d{10}$/, 'Mobile number must be 10 digits').optional().or(z.literal('')),
  email: z.string().email('Invalid email format').optional().or(z.literal('')),
  alternateEmail1: z.string().email('Invalid email format').optional().or(z.literal('')),
  alternateEmail2: z.string().email('Invalid email format').optional().or(z.literal('')),
  alternateEmail3: z.string().email('Invalid email format').optional().or(z.literal('')),
  alternateEmail4: z.string().email('Invalid email format').optional().or(z.literal('')),
...
export function VendorForm({ mode, initial, onSuccess, redirectOnSuccess }: VendorFormProps) {
  ...
  const { data: itemCategories } = useSWR('/api/item-categories?perPage=100', apiGet);
  ...
  const stateOptions = (states as any)?.data?.map((state: any) => ({
    value: String(state.id),
    label: state.state,
  })) || [];

  const cityOptions = (cities as any)?.data?.map((city: any) => ({
    value: String(city.id),
    label: city.city,
  })) || [];

  const itemCategoryOptions = (itemCategories as any)?.data?.map((category: any) => ({
    ...
    <>
      ...
      <Form {...form}>
        ...
        <AppCard>
          ...
          <AppCard.Content>
            <FormSection legend={<span className='text-base font-semibold'>Basic Information</span>}>
            ...
            <FormSection legend={<span className='text-base font-semibold'>Item Categories</span>}>
              <FormRow>
                <MultiSelectInput 
                  control={control} 
