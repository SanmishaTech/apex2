...
export default function WageSheetPage() {
  ...
  async function exportPdf() {
    ...
    try {
      ...
      const pdfBlob = await generateWageSheetPDF(wageData, period, siteId !== "all" ? sites.data?.data?.find((s: any) => String(s.id) === siteId)?.site : "All Sites");
  ...
  function generateWageSheetPDF(wageData: any, period: string, siteName: string) {
    const doc = new jsPDF({ orientation: "landscape", unit: "mm", format: "a3" });
    
    const daysInMonth = wageData.daysInMonth || 31;
    const [mm, yyyy] = period.split('-');
    const monthName = new Date(Number(yyyy), Number(mm) - 1).toLocaleString('en', { month: 'long' });
    ...
    doc.setFontSize(12);
    doc.text(`Report : Monthly Wage Sheet As Per ${mode === 'govt' ? 'Minimum Wage' : 'Company Rates'}`, 14, 22);
    ...
    wageData.data.forEach((siteGroup: any, siteIdx: number) => {
      ...
      if (mode === 'company') {
        ...
        Object.entries(supplierGroups).forEach(([supplier, workers]) => {
          groupedWorkers.push({ isSupplierHeader: true, supplierName: supplier });
      ...
      // Build table body
      const body: any[] = [];
      let totalGross = 0, totalHra = 0, totalPf = 0, totalEsic = 0, totalPt = 0, totalLwf = 0, totalDeduction = 0, totalPayable = 0;
      let totalWorkingDays = 0, totalOT = 0, totalActualWages = 0, totalIdleDays = 0, totalIdleWages = 0, totalTotalWages = 0;
      ...
      groupedWorkers.forEach((item: any, idx: number) => {
        if (item.isSupplierHeader) {
          ...
          const supplierHeaderRow = new Array(6 + daysInMonth + (mode === 'govt' ? 10 : 6)).fill('');
          supplierHeaderRow[1] = item.supplierName; // Put supplier name in second column
          body.push(supplierHeaderRow);
          
          // Reset supplier totals
          supplierTotals = { workingDays: 0, ot: 0, actualWages: 0, idleDays: 0, idleWages: 0, totalWages: 0 };
          return;
        }
        ...
        if (mode === 'govt') {
          ...
          totalIdleDays += Number(worker.idleDays || 0) + Number(worker.idleOT || 0);
          ...
          if (supplierTotals) {
            ...
            supplierTotals.idleDays += Number(worker.idleDays || 0) + Number(worker.idleOT || 0);
      ...
      autoTable(doc, {
        ...
        didParseCell: (hookData: any) => {
          if (hookData.section === "body") {
            ...
            if (mode === 'company' && hookData.column.index === 1 && 
            ...
            if ((cellText === "Total" || cellText === "Grand Total") && hookData.column.index === 1) {
              hookData.row.cells.forEach((cell: any) => {
                cell.styles.fontStyle = "bold";
                cell.styles.fillColor = cellText === "Grand Total" ? [200, 200, 200] : [230, 230, 230];
              });
            }
          }
        },
