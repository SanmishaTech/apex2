...
const createSchema = createInputSchema.transform(data => ({
  ...
  resignDate: data.resignDate && data.resignDate !== '' ? new Date(data.resignDate) : null,
  dateOfBirth: data.dateOfBirth && data.dateOfBirth !== '' ? new Date(data.dateOfBirth) : null,
  anniversaryDate: data.anniversaryDate && data.anniversaryDate !== '' ? new Date(data.anniversaryDate) : null,
  // Travel/Reporting Details
  reporting1Id: data.reporting1Id && data.reporting1Id !== '' ? parseInt(data.reporting1Id) : null,
  reporting2Id: data.reporting2Id && data.reporting2Id !== '' ? parseInt(data.reporting2Id) : null,
  // Reporting Sites
  reportingSiteId: data.reportingSiteId && data.reportingSiteId !== '' ? parseInt(data.reportingSiteId) : null,
  reportingSiteAssignedDate: data.reportingSiteAssignedDate && data.reportingSiteAssignedDate !== '' ? new Date(data.reportingSiteAssignedDate) : null,
  // Leave Details - convert to numbers
  sickLeavesPerYear: data.sickLeavesPerYear && data.sickLeavesPerYear !== '' ? parseInt(data.sickLeavesPerYear) : null,
  paidLeavesPerYear: data.paidLeavesPerYear && data.paidLeavesPerYear !== '' ? parseInt(data.paidLeavesPerYear) : null,
  casualLeavesPerYear: data.casualLeavesPerYear && data.casualLeavesPerYear !== '' ? parseInt(data.casualLeavesPerYear) : null,
  balanceSickLeaves: data.balanceSickLeaves && data.balanceSickLeaves !== '' ? parseInt(data.balanceSickLeaves) : null,
  balancePaidLeaves: data.balancePaidLeaves && data.balancePaidLeaves !== '' ? parseInt(data.balancePaidLeaves) : null,
  balanceCasualLeaves: data.balanceCasualLeaves && data.balanceCasualLeaves !== '' ? parseInt(data.balanceCasualLeaves) : null,
}));
...
export function EmployeeForm({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      anniversaryDate: initial?.anniversaryDate ? initial.anniversaryDate.split('T')[0] : '',
      ...
      // Reporting Sites
      reportingSiteId: initial?.reportingSiteId ? String(initial.reportingSiteId) : '',
      reportingSiteAssignedDate: initial?.reportingSiteAssignedDate ? initial.reportingSiteAssignedDate.split('T')[0] : '',
      // Leave Details
      sickLeavesPerYear: '',
      paidLeavesPerYear: '',
      casualLeavesPerYear: '',
  ...
  const { data: citiesData } = useSWR<CitiesResponse>(
    selectedStateId && selectedStateId !== '' ? `/api/cities?stateId=${selectedStateId}&perPage=100` : null,
    apiGet
  );
    ...
    try {
      ...
      if (isCreate) {
        ...
        if (signatureFile || profilePicFile) {
          ...
          if (data.departmentId) fd.append('departmentId', String(data.departmentId));
          if (data.siteId) fd.append('siteId', String(data.siteId));
          if (data.resignDate) fd.append('resignDate', data.resignDate.toISOString());
          if (data.dateOfBirth) fd.append('dateOfBirth', data.dateOfBirth.toISOString());
          if (data.anniversaryDate) fd.append('anniversaryDate', data.anniversaryDate.toISOString());
          ...
          // Travel/Reporting Details
          if (data.airTravelClass) fd.append('airTravelClass', data.airTravelClass);
          if (data.railwayTravelClass) fd.append('railwayTravelClass', data.railwayTravelClass);
          if (data.busTravelClass) fd.append('busTravelClass', data.busTravelClass);
          if (data.reporting1Id) fd.append('reporting1Id', String(data.reporting1Id));
          if (data.reporting2Id) fd.append('reporting2Id', String(data.reporting2Id));
          // Reporting Sites
          if (data.reportingSiteId) fd.append('reportingSiteId', String(data.reportingSiteId));
          if (data.reportingSiteAssignedDate) fd.append('reportingSiteAssignedDate', data.reportingSiteAssignedDate.toISOString());
          ...
          const response = await fetch('/api/employees', { method: 'POST', body: fd });
          if (!response.ok) {
            const err = await response.json().catch(() => null);
            throw new Error(err?.message || `HTTP ${response.status}: Failed to save employee`);
          }
        ...
        submitData = {
          ...
          reportingSiteAssignedDate: data.reportingSiteAssignedDate?.toISOString() || undefined,
  ...
  return (
    <Form {...form}>
      <AppCard>
        <AppCard.Header>
          <AppCard.Title>{isCreate ? 'Create Employee' : 'Edit Employee'}</AppCard.Title>
          <AppCard.Description>
            {isCreate ? 'Add a new employee to the system.' : 'Update employee information.'}
          </AppCard.Description>
        </AppCard.Header>
