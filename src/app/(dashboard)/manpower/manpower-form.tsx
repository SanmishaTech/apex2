...
export default function ManpowerForm({ mode, initial, onSuccess, redirectOnSuccess = '/manpower' }: ManpowerFormProps) {
  ...
  const { data: suppliers } = useSWR<SuppliersResponse>('/api/manpower-suppliers?perPage=100', apiGet);
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      ...
      if (values.accountNumber) fd.append('accountNumber', values.accountNumber);
      ...
      if (values.drivingLicenceNo) fd.append('drivingLicenceNo', values.drivingLicenceNo);
      if (values.watch) fd.append('watch', String(values.watch));
      // docs
      if (values.panDocument) fd.append('panDocument', values.panDocument);
      if (values.aadharDocument) fd.append('aadharDocument', values.aadharDocument);
      if (values.voterIdDocument) fd.append('voterIdDocument', values.voterIdDocument);
      if (values.drivingLicenceDocument) fd.append('drivingLicenceDocument', values.drivingLicenceDocument);
      if (values.bankDetailsDocument) fd.append('bankDetailsDocument', values.bankDetailsDocument);

      let res: any;
      if (isCreate) {
        const resp = await fetch('/api/manpower', { method: 'POST', body: fd });
        const data = await resp.json().catch(() => null);
        if (!resp.ok) throw new Error(data?.message || 'Failed to create manpower');
        res = data;
        toast.success('Manpower created');
      } else if (initial?.id) {
        fd.append('id', String(initial.id));
        const resp = await fetch('/api/manpower', { method: 'PATCH', body: fd });
        const data = await resp.json().catch(() => null);
        if (!resp.ok) throw new Error(data?.message || 'Failed to update manpower');
        res = data;
  ...
  return (
    <Form {...form}>
      <AppCard>
        <AppCard.Header>
          <AppCard.Title>{isCreate ? 'Create Manpower' : 'Edit Manpower'}</AppCard.Title>
          <AppCard.Description>
            {isCreate ? 'Add a new manpower worker.' : 'Update manpower details.'}
          </AppCard.Description>
        </AppCard.Header>
          ...
          <AppCard.Content>
            <FormSection legend='Personal Details'>
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='firstName' label='First Name' placeholder='First name' required itemClassName='col-span-4' />
                <TextInput control={control} name='middleName' label='Middle Name' placeholder='Middle name' itemClassName='col-span-4' />
              ...
              <FormRow className='grid-cols-12'>
                ...
                <TextInput control={control} name='dateOfBirth' label='Date of Birth' type='date' placeholder='YYYY-MM-DD' itemClassName='col-span-6' />
              </FormRow>
            </FormSection>

            <FormSection legend='Contact Details'>
              <FormRow className='grid-cols-12'>
                <TextareaInput control={control} name='address' label='Address' placeholder='Full address' itemClassName='col-span-6' />
                <TextInput control={control} name='location' label='Location' placeholder='Location' itemClassName='col-span-6' />
              </FormRow>
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='mobileNumber' label='Mobile Number' placeholder='Mobile number' itemClassName='col-span-6' />
                <TextInput control={control} name='wage' label='Wage' placeholder='0.00' itemClassName='col-span-6' />
              </FormRow>
            </FormSection>

            <FormSection legend='Bank Details'>
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='bank' label='Bank' placeholder='Bank name' itemClassName='col-span-6' />
                <TextInput control={control} name='branch' label='Branch' placeholder='Branch' itemClassName='col-span-6' />
              </FormRow>
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='accountNumber' label='Account Number' placeholder='Account number' itemClassName='col-span-6' />
                <TextInput control={control} name='ifscCode' label='IFSC Code' placeholder='IFSC code' itemClassName='col-span-6' />
              </FormRow>
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='pfNo' label='PF No.' placeholder='PF number' itemClassName='col-span-6' />
                <TextInput control={control} name='esicNo' label='ESIC No.' placeholder='ESIC number' itemClassName='col-span-6' />
              </FormRow>
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='unaNo' label='UNA No.' placeholder='UNA number' itemClassName='col-span-6' />
              </FormRow>
            </FormSection>

            <FormSection legend='Other Details'>
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='panNumber' label='PAN Number' placeholder='PAN' itemClassName='col-span-6' />
                <UploadInput
                  control={control}
                  name='panDocument'
                  label='PAN Document'
              ...
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='aadharNo' label='Aadhar No' placeholder='Aadhar number' itemClassName='col-span-6' />
                <UploadInput
                  control={control}
                  name='aadharDocument'
              ...
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='voterIdNo' label='Voter Id No' placeholder='Voter Id number' itemClassName='col-span-6' />
                <UploadInput
                  control={control}
              ...
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='drivingLicenceNo' label='Driving Licence No' placeholder='DL number' itemClassName='col-span-6' />
                <UploadInput
                  control={control}
              ...
              <FormRow className='grid-cols-12'>
                <TextInput control={control} name='bankDetails' label='Bank Details' placeholder='Bank details' itemClassName='col-span-6' />
                <UploadInput
                  control={control}
              ...
              <FormRow className='grid-cols-12'>
                <div className='col-span-6'>
                  <AppCheckbox label='Watch' checked={!!form.watch('watch')} onCheckedChange={(v) => form.setValue('watch', v)} />
                </div>
                <div className='col-span-6' />
              </FormRow>
            </FormSection>
