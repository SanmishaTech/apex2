...
export async function GET(req: NextRequest) {
  ...
  const dailyMap = new Map<number, Map<number, Map<number, { isPresent: boolean; isIdle: boolean; ot: number }>>>();
  ...
  for (const detail of paySlipDetails) {
    ...
    siteGroup.workers.push({
      ...
      idleOT: Number(detail.idleOt || 0),
      wageRate: Number(detail.wages || 0),
      grossWage: Number(detail.grossWages || 0),
      actualWages: Number(detail.total || 0),
      idleWages: Number(detail.wages || 0) * (Number(detail.idle || 0) + Number(detail.idleOt || 0)),
      totalWages: Number(detail.total || 0) + (Number(detail.wages || 0) * (Number(detail.idle || 0) + Number(detail.idleOt || 0))),
      hra: Number(detail.hra || 0),
      pf: Number(detail.pf || 0),
      esic: Number(detail.esic || 0),
      pt: Number(detail.pt || 0),
      lwf: Number(detail.mlwf || 0),
      totalDeduction: Number(detail.hra || 0) + Number(detail.pf || 0) + Number(detail.esic || 0) + Number(detail.pt || 0) + Number(detail.mlwf || 0),
