...
export async function POST(req: NextRequest) {
  ...
  if (existingBudget) {
    return Error(
      "Budget for this month, site, and BOQ combination already exists",
      409
    );
  }
  ...
  try {
    ...
    if (err?.code === "P2002") {
      return Error(
        "Month must be unique or budget of this month already exists",
        409
      );
    }
