  ...
  try {
    ...
    if (parsed.statusAction === "approve") {
      ...
      for (const row of parsed.outwardDeliveryChallanDetails || []) {
        ...
        if (qty <= 0) {
          return BadRequest(`Approved qty must be greater than 0 for detail ${row.id}`);
        }
        if (qty > Number(found.challanQty || 0)) {
          return BadRequest(
            `Approved qty cannot exceed challan qty for detail ${row.id}`
          );
        }
      }
      ...
      await prisma.$transaction(async (tx) => {
        ...
        for (const row of parsed.outwardDeliveryChallanDetails || []) {
          await tx.outwardDeliveryChallanDetail.update({
            where: { id: row.id },
            data: { approved1Qty: Number(row.approved1Qty ?? 0) },
          });
        }
      });
      ...
      for (const row of parsed.outwardDeliveryChallanDetails || []) {
        ...
        if (qty <= 0) {
          return BadRequest(`Received qty must be greater than 0 for detail ${row.id}`);
        }
        const max = Number(
          (found.approved1Qty != null ? found.approved1Qty : found.challanQty) || 0
        );
        if (qty < 0 || qty > max) {
          return BadRequest(
            `Received qty cannot exceed approved qty for detail ${row.id}`
          );
        }
      }
      ...
      await prisma.$transaction(async (tx) => {
        ...
        for (const row of parsed.outwardDeliveryChallanDetails || []) {
          await tx.outwardDeliveryChallanDetail.update({
            where: { id: row.id },
            data: { receivedQty: Number(row.receivedQty ?? 0) },
          });
        }
      });
...
// DELETE /api/outward-delivery-challans/[id]
export async function DELETE(
  ...
  try {
    ...
    const challan = await prisma.outwardDeliveryChallan.findUnique({
      ...
      select: {
        id: true,
        outwardDeliveryChallanDocuments: { select: { documentUrl: true } },
      },
    });
    ...
    await prisma.$transaction(async (tx) => {
      await tx.outwardDeliveryChallanDocuments.deleteMany({
        where: { outwardDeliveryChallanId: id },
      });
      await tx.outwardDeliveryChallanDetail.deleteMany({
        where: { outwardDeliveryChallanId: id },
      });
      await tx.outwardDeliveryChallan.delete({ where: { id } });
    });

    for (const doc of challan.outwardDeliveryChallanDocuments) {
      if (doc.documentUrl.startsWith("/uploads/outward-delivery-challans/")) {
    ...
    return Success({
      message: "Outward delivery challan deleted successfully",
    });
  } catch (error: unknown) {
    if (
      error &&
      ...
      return NotFound("Outward delivery challan not found");
    }
    console.error("Delete outward delivery challan error:", error);
    return ApiError("Failed to delete outward delivery challan");
  }
}
