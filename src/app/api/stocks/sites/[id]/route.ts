...
export async function GET(
  ...
  try {
    ...
    const siteItems = await prisma.siteItem.findMany({
      where: { siteId: id },
      select: { itemId: true, closingStock: true, item: { select: { item: true, unit: { select: { unitName: true } } } } },
    });
    ...
    const ledgersInRange = await prisma.stockLedger.findMany({
      ...
      select: { itemId: true, transactionDate: true, receivedQty: true, issuedQty: true },
    });
    ...
    const openingAgg = await prisma.stockLedger.groupBy({
      by: ["itemId"],
      where: { siteId: id, itemId: { in: itemIds }, transactionDate: { lt: startDate } },
    ...
    const dailyMap = new Map<number, Map<string, { received: number; issued: number }>>();
    for (const l of ledgersInRange) {
      const dKey = startOfDay(new Date(l.transactionDate)).toISOString().slice(0, 10);
      ...
      prev.received = Number((prev.received + Number(l.receivedQty || 0)).toFixed(4));
      prev.issued = Number((prev.issued + Number(l.issuedQty || 0)).toFixed(4));
      m.set(dKey, prev);
    }

    const missingItemIds = itemIds.filter((iid) => !siteItems.some((s) => s.itemId === iid));
    const missingItemsMeta = missingItemIds.length
      ? await prisma.item.findMany({ where: { id: { in: missingItemIds } }, select: { id: true, item: true, unit: { select: { unitName: true } } } })
      : [];

    const metaByItem = new Map<number, { name: string; unit: string | null; closing?: number }>();
    for (const s of siteItems) metaByItem.set(s.itemId, { name: s.item.item, unit: s.item.unit?.unitName ?? null, closing: Number(s.closingStock || 0) });
    for (const mi of missingItemsMeta) if (!metaByItem.has(mi.id)) metaByItem.set(mi.id, { name: mi.item, unit: mi.unit?.unitName ?? null });

    const dayKeys = days.map((d) => d.toISOString().slice(0, 10));
    const rows = itemIds.map((itemId) => {
      const meta = metaByItem.get(itemId) || { name: `Item ${itemId}`, unit: null };
      const opening = openingByItem.get(itemId) ?? 0;
      let recvTotal = 0;
      let issuedTotal = 0;
      const perDay = dayKeys.map((dk) => {
        const cell = dailyMap.get(itemId)?.get(dk) || { received: 0, issued: 0 };
        recvTotal += cell.received;
        issuedTotal += cell.issued;
        return { date: dk, received: cell.received, issued: cell.issued };
      });
      const closing = Number((opening + recvTotal - issuedTotal).toFixed(4));
      return { itemId, item: meta.name, unit: meta.unit, opening, perDay, closing };
    });
