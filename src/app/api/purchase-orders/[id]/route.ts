    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (Object.keys(poData).length > 0 || statusAction) {
        ...
        if (statusAction) {
          ...
          // Validate against SiteBudget on approval actions
          if (
            (statusAction === "approve1" || statusAction === "approve2") &&
            !ignoreBudgetValidation
          ) {
            const nameById = new Map<number, string>();
            for (const d of (current.purchaseOrderDetails || []) as Array<{
              itemId: number;
              item?: { item?: string | null };
            }>) {
              nameById.set(Number(d.itemId), d.item?.item ?? String(d.itemId));
            }

            let poItems = (current.purchaseOrderDetails || []) as Array<{
              itemId: number;
              qty: number | null;
              rate?: number | null;
              item?: { item?: string | null };
            ...
            if (
              ...
              poItems = (purchaseOrderItems as any[]).map((it) => ({
                itemId: Number(it.itemId),
                qty: it.qty != null ? Number(it.qty) : null,
                rate: it.rate != null ? Number(it.rate) : null,
                item: {
                  item: nameById.get(Number(it.itemId)) || String(it.itemId),
                },
              }));
            }

            // Aggregate requested qty per itemId
            const requestedMap = new Map<
              number,
              { qty: number; name: string }
            >();
            for (const d of poItems) {
              const idNum = Number(d.itemId);
              const q = Number(d.qty || 0);
              if (!Number.isFinite(idNum) || q <= 0) continue;
              const prev = requestedMap.get(idNum);
              const name = d.item?.item ?? String(idNum);
              if (prev) {
                prev.qty += q;
              } else {
                requestedMap.set(idNum, { qty: q, name });
              }
            }

            if (requestedMap.size > 0) {
              const itemIds = Array.from(requestedMap.keys());
              const budgets = await tx.siteBudget.findMany({
                where: { siteId: current.siteId, itemId: { in: itemIds } },
                select: {
                  itemId: true,
                  budgetQty: true,
                  orderedQty: true,
              ...
              for (const b of budgets) {
                const remaining = Math.max(
                  0,
                  Number(b.budgetQty) - Number(b.orderedQty || 0)
                );
                const remainingValue = Math.max(
                  0,
                  Number(b.budgetValue || 0) - Number(b.orderedValue || 0)
                );
                budgetMap.set(Number(b.itemId), {
                  remaining,
              ...
              for (const [itemId, { qty, name }] of requestedMap.entries()) {
                const remaining = budgetMap.get(itemId)?.remaining ?? 0;
                if (qty > remaining) {
                  const fmt = (n: number) =>
                    Number.isInteger(n) ? String(n) : n.toFixed(2);
                  violations.push(`${name}: ${fmt(qty)}/${fmt(remaining)}`);
                }
              }

              // Rate validation: compare max entered rate per item vs budgetRate
              const maxRateByItem = new Map<
                number,
              ...
              for (const d of poItems) {
                ...
                const name = d.item?.item ?? String(idNum);
                const prev = maxRateByItem.get(idNum);
                if (!prev || r > prev.rate) {
                  maxRateByItem.set(idNum, { rate: r, name });
                }
              }

              const rateViolations: string[] = [];
              for (const [itemId, { rate, name }] of maxRateByItem.entries()) {
                const budgetRate = budgetMap.get(itemId)?.budgetRate ?? 0;
                if (rate > budgetRate) {
                  const fmt = (n: number) =>
                    Number.isInteger(n) ? String(n) : n.toFixed(2);
                  rateViolations.push(
                    `${name}: ${fmt(rate)}/${fmt(budgetRate)}`
                  );
                }
              }

              // Value validation: compare sum of (qty*rate) vs remainingValue
              const valueRequested = new Map<
                number,
              ...
              for (const d of poItems) {
                ...
                const name = d.item?.item ?? String(idNum);
                const add = q * r;
                const prev = valueRequested.get(idNum);
                if (prev) prev.value += add;
                else valueRequested.set(idNum, { value: add, name });
              }
              const valueViolations: string[] = [];
              for (const [
                itemId,
                { value, name },
              ] of valueRequested.entries()) {
                const remainingValue =
                  budgetMap.get(itemId)?.remainingValue ?? 0;
                if (value > remainingValue) {
                  const fmt = (n: number) =>
                    Number.isInteger(n) ? String(n) : n.toFixed(2);
                  valueViolations.push(
                    `${name}: ${fmt(value)}/${fmt(remainingValue)}`
                  );
                }
              }
              ...
              if (
                ...
                if (violations.length > 0) {
                  parts.push(
                    `Item limit exceeded. Requested/Available for items -> ${violations.join(
                      ", "
                    )}`
                  );
                }
                if (rateViolations.length > 0) {
                  parts.push(
                    `Rate limit exceeded. Entered/Budget for items -> ${rateViolations.join(
                      ", "
                    )}`
                  );
                }
                if (valueViolations.length > 0) {
                  parts.push(
                    `Value limit exceeded. Entered/Budget for items -> ${valueViolations.join(
                      ", "
                    )}`
                  );
                }
                throw new Error(`BAD_REQUEST: ${parts.join(" | ")}`);
              }
            }
          }
      ...
      // If this request is an approval action, recompute SiteBudget.orderedQty for this site
      if (statusAction === "approve1" || statusAction === "approve2") {
        // Get siteId for this PO (avoid relying on outer-scoped vars)
        const poSite = await tx.purchaseOrder.findUnique({
          where: { id },
          select: { siteId: true },
        });
        const siteId = poSite?.siteId;
        if (!siteId) {
          // If no siteId, skip budget aggregation safely
        } else {
          const posForSite = await tx.purchaseOrder.findMany({
            where: {
              siteId,
              approvalStatus: { in: ["APPROVED_LEVEL_1", "APPROVED_LEVEL_2"] },
            },
            select: {
              id: true,
              purchaseOrderDetails: {
          ...
          // Aggregate total approved quantity per itemId
          const totalByItem = new Map<number, number>();
          const totalValueByItem = new Map<number, number>();
          for (const po of posForSite) {
            for (const d of po.purchaseOrderDetails) {
              const idNum = Number(d.itemId);
              const q = Number(d.approved2Qty ?? d.approved1Qty ?? d.qty ?? 0);
              if (!Number.isFinite(idNum) || q <= 0) continue;
              const prev = totalByItem.get(idNum) || 0;
              totalByItem.set(idNum, prev + q);

              const amt = Number(d.amount || 0);
              const prevVal = totalValueByItem.get(idNum) || 0;
              totalValueByItem.set(idNum, prevVal + amt);
            }
          }

          if (totalByItem.size > 0) {
            const itemIds = Array.from(totalByItem.keys());
            const budgets = await tx.siteBudget.findMany({
            ...
            for (const itemId of itemIds) {
              ...
              if (found) {
                await tx.siteBudget.update({
                  where: { id: found.id },
                  data: { orderedQty, orderedValue, avgRate },
                });
              } else {
                await tx.siteBudget.create({
                  data: {
                    siteId,
                    itemId,
                    budgetQty: 0,
