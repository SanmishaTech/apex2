...
// DELETE /api/inward-delivery-challans/[id] - Delete inward delivery challan
export async function DELETE(
  ...
  try {
    ...
    // Get the challan with its documents to delete files
    const challan = await prisma.inwardDeliveryChallan.findUnique({
      where: { id },
      select: {
        id: true,
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Delete the main challan
      await tx.inwardDeliveryChallan.delete({
        where: { id },
      });
    });

    // Delete physical document files
    for (const doc of challan.inwardDeliveryChallanDocuments) {
      if (doc.documentUrl.startsWith("/uploads/inward-delivery-challans/")) {
        const filePath = path.join(process.cwd(), doc.documentUrl);
        try {
          await fs.unlink(filePath);
    ...
    return Success({ message: "Inward delivery challan deleted successfully" });
  } catch (error: unknown) {
    if (
      error &&
      typeof error === "object" &&
    ...
    console.error("Delete inward delivery challan error:", error);
    return ApiError("Failed to delete inward delivery challan");
  }
}
